...
class CSI_Camera:
    ...
    def stop(self):
        ...
        if self.output_thread:
            self.output_queue.put(self.frame)
    ...
    def write_to_file(self):
        ...
        fps_ = FPS().start()
        while self.running:
            frame_ = self.output_queue.get().copy()
            cv2.putText(frame_, str(datetime.now()), (5, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 255, 255), 1, cv2.LINE_AA)
